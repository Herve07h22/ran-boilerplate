...
const base = Airtable.base(process.env.AIRTABLE_DATABASE);
...
const query = (userEmail, view) => {
  const allRecords = [];
  const userIsInRecords = (records) =>
    records.some((r) => r["Email"] === userEmail) ? records : [];
...
exports.handler = (event, context, callback) => {
  ...
  checkAuth(context)
    .then((user) => {
      ...
      query(user.email, "Membres").then((records) =>
        callback(null, {
          statusCode: 200,
          headers,
          body: JSON.stringify(
            records && records.length
              ? { cards: records }
              : { error: "Désolé, votre fiche n'est pas encore validée." }
          ),
        })
      );
    })
